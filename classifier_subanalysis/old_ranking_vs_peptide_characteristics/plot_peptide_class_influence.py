...
def run_main_barplot(plot_p, models_to_keep):
    ...
    for model_i, model in enumerate(sorted(models_to_keep)):
        ...
        # sort by class
        model_df = model_df.sort_values("Class")
        # plot
        sns.barplot(
            x="Class",
            y="AgonistRank",
        ...
        mean_rank_dict = model_df.groupby("Class")["AgonistRank"].mean().to_dict()
        # add text
        ax.set_title(f"Δˉ(A-B): {mean_rank_dict[gpcr_classes[0]] - mean_rank_dict[gpcr_classes[1]]:.2f}",
                     fontsize=8, pad=0,
                     # text left
                       loc="right")
        ax.set_xlabel("")
...
if __name__ == "__main__":
    ...
    run_main_barplot(plot_p = script_dir / "ranking_vs_class/class_comparisons_barplot.svg",
