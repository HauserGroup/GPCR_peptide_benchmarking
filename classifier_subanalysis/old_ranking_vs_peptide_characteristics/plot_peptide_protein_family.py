...
def run_main_barplot(plot_p, models_to_keep):
    ...
    for model_i, model in enumerate(sorted(models_to_keep)):
        ...
        # sort by class
        model_df = model_df.sort_values("Family")
        # plot
        sns.barplot(
            x="Family",
            y="AgonistRank",
        ...
        mean_rank_dict = model_df.groupby("Family")["AgonistRank"].mean().to_dict()
        # add text
        ax.set_title(f"Δˉ(Protein-Peptide): {mean_rank_dict['Protein'] - mean_rank_dict['Peptide']:.2f}",
                     fontsize=8, pad=0,
                     # text left
                       loc="right")
        ax.set_xlabel("")
...
if __name__ == "__main__":
    ...
    run_main_barplot(plot_p = script_dir / "ranking_vs_family/family_comparisons_barplot.svg",
