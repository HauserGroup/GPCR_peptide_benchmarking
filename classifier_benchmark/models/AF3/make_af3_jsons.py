...
os.makedirs(f"{repo_dir}/classifier_benchmark/models/AF3/classifier_jsons/complexes", exist_ok=True)
...
for index, row in decoy_df.iterrows():
    ...
    # Make json for the complex
    complex_dict = {}
    complex_dict["name"] = identifier
    complex_dict["sequences"] = []
    complex_dict["sequences"].append({"protein": {"id": "A", "sequence": gpcr_sequence}})
    complex_dict["sequences"].append({"protein": {"id": "B", "sequence": ligand_sequence}})
    complex_dict["modelSeeds"] = [1]
    complex_dict["dialect"] = "alphafold3"
    complex_dict["version"] = 1
    with open(f"{repo_dir}/classifier_benchmark/models/AF3/classifier_jsons/complexes/{identifier}.json", "w") as f:
