...
def run_main():
    ...
    fig, axs = plt.subplots(
        len(unique_models), 1, figsize=(len(unique_models), 2 * len(unique_models))
    )
    ...
    for model_index, (model_name, prediction_df) in enumerate(models):
        prediction_df["gpcr_id"] = prediction_df["identifier"].apply(
            lambda x: x.split("___")[0]
        )
        prediction_df["class"] = prediction_df["gpcr_id"].apply(
            lambda x: gpcr_to_class[x]
        )
        prediction_df["gpcr"] = prediction_df["gpcr_id"].apply(
            lambda x: x.split("_human")[0]
        )

        # sort on class, then gpcr
        prediction_df = prediction_df.sort_values(["class", "gpcr"])
        y_true = get_ground_truth_values(
            ground_truth=ground_truth, identifiers=prediction_df["identifier"].values
        )
        ...
        plot_score_dist(
            ...
            y_pred,
            y_true,
            prediction_df["gpcr"].values,
            prediction_df["class"].values,
            ax,
        )
...
if __name__ == "__main__":
    GPCR_TO_COLOR = {
        "Class A (Rhodopsin) PA": "rgba(17,81,133,0.4)",
